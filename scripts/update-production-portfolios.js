...
async function updateProductionPortfolios() {
  try {
    ...
    console.log('Database URL:', process.env.DATABASE_URL ? 'Connected to production' : 'ERROR: No production URL found');
    console.log('');
    ...
    if (existingArtists.length === 0) {
      console.log('❌ No makeup artists found in production database.');
      await prisma.$disconnect();
      return;
    }
    
    console.log(`✅ Found ${existingArtists.length} makeup artists in production database.\n`);
    ...
    console.log('🌱 Starting production portfolio image update...\n');
    ...
    await prisma.$transaction(async (tx) => {
      ...
      console.log(`\n✨ Successfully updated ${updatedCount} artist portfolios in PRODUCTION!`);
    });
    
    console.log('\n🎉 Production portfolio update completed successfully!');
    console.log('\nAll artist portfolios now feature makeup-specific images from Unsplash.');
...
// Run the update
console.log('=== PRODUCTION PORTFOLIO UPDATE ===\n');
console.log('This script will update all makeup artist portfolio images in the PRODUCTION database.\n');
