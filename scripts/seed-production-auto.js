...
// Makeup artist data (same as before)
const makeupArtists = [
  {
...
async function seedProduction() {
  try {
    ...
    console.log(`Force: ${forceMode ? 'YES' : 'NO'}\n`);
    
    // Check if artists already exist
    const existingArtists = await prisma.makeupArtist.count();
    
    if (existingArtists > 0 && !forceMode) {
      console.log(`⚠️  Database already contains ${existingArtists} makeup artists.`);
      console.log('Use --force flag to add more artists anyway.');
      await prisma.$disconnect();
      process.exit(0);
    }
    
    if (dryRun) {
      console.log('📋 DRY RUN - No data will be added:\n');
      ...
      console.log('Run without --dry-run flag to actually seed the database.');
      await prisma.$disconnect();
      process.exit(0);
    }
    ...
    // Seed artists one by one (no transaction due to timeout issues)
    let createdCount = 0;
    ...
    for (const artistData of makeupArtists) {
      ...
      try {
        ...
        const existingArtist = await prisma.makeupArtist.findUnique({
          where: { username: artistData.username }
        });
        
        if (existingArtist) {
          console.log(`⏭️  Skipping ${artistData.name} - already exists`);
          continue;
        }
        ...
        for (let i = 0; i < reviews.length; i++) {
          ...
          // Create a dummy client for the review
          const reviewClient = await prisma.client.upsert({
        ...
        console.error(`❌ Failed to create ${artistData.name}:`, error.message);
      }
    }
...
if (args.includes('--help')) {
  console.log('Usage: node seed-production-auto.js [options]');
  console.log('\nOptions:');
  console.log('  --dry-run   Show what would be seeded without making changes');
  console.log('  --force     Seed even if data already exists');
  console.log('  --help      Show this help message');
  console.log('\nEnvironment variables:');
  console.log('  DATABASE_URL or POSTGRES_URL - Required database connection string');
  process.exit(0);
}
